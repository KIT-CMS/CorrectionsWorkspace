...
loc = 'inputs/2016/singletau/DESY/'
    ...
    for trg in ["singleANDdouble_t", "single_t", "double_t"]:
        for dm in ["1pr", "1pr1pi0", "3pr"]:
            for region in ["barrel", "endcap"]:
                histsToWrap.append(
                        (loc + 'eff_tauTriggers_2016_v2.root:trg_{trg}{sample}_{dm}_{reg}'.format(trg=trg,
                                                                                               sample=sample,
                                                                                               dm=dm,
                                                                                               reg=region),
...
histsToWrap = [
        (loc + 'SingleTauTriggerEff_MediumDeepTau2017v2p1_2016.root:MC', 't_trg_single_t_wstar_mc'),
        (loc + 'SingleTauTriggerEff_MediumDeepTau2017v2p1_2016.root:Data', 't_trg_single_t_wstar_data'),
        (loc + 'SingleTauTriggerEff_MediumDeepTau2017v2p1_2016.root:SF', 't_trg_single_t_wstar_ratio')
]
for task in histsToWrap:
    wsptools.SafeWrapHist(w, ['t_pt'],
                          GetFromTFile(task[0]), name=task[1])
...
w.Print()
w.writeToFile('output/htt_scalefactors_singletau_legacy_2016.root')
w.Delete()
